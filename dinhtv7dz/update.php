...
if (isset($_GET["id_user"])) {
    ...
    // Nếu không có dữ liệu về người dùng
    if (empty($userDetail)) {
        echo "Người không tồn tại";
        exit();
    } else {
...
<body>
    ...
    <form action="" method="POST">
        ...
        <input type="text" name="ho_ten" value="<?= $userDetail["name"] ?>">
        ...
        <select name="trang_thai">
            <option value="0" <?= $userDetail["status"] == 0 ? 'selected' : '' ?>>Ngừng hoạt động</option>
            <option value="1" <?= $userDetail["status"] == 1 ? 'selected' : '' ?>>Hoạt động</option>
        </select>
        <br>
